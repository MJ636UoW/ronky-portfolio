  ...
  const initWPContactFormAJAX = () => {
    ...
    contactForms.forEach(contactForm => {
      ...
      contactForm.addEventListener('submit', (e) => {
        ...
        formData.append('action', 'ronky_submit_custom_contact_form');

        const formId = contactForm.dataset.fluentFormId || '';
        if (formId) {
          formData.append('fluent_form_id', formId);
        }
        ...
        formData.append('service', serviceInput ? serviceInput.value : '');
        formData.append('budget', budgetInput ? budgetInput.value : '');
        formData.append('message', messageInput ? messageInput.value : '');

        // Fetch URL
        let ajaxUrl = '/wp-admin/admin-ajax.php';
        if (typeof genz_ajax !== 'undefined' && genz_ajax.ajax_url) {
            ajaxUrl = genz_ajax.ajax_url;
        } else if (typeof window.ajaxurl !== 'undefined' && window.ajaxurl) {
            ...
            for (let i = 0; i < scripts.length; i++) {
                ...
                if (src && src.indexOf('/wp-content/') !== -1) {
                    ajaxUrl = src.substring(0, src.indexOf('/wp-content/')) + '/wp-admin/admin-ajax.php';
                    break;
                }
            }
        }
        ...
        .then(res => {
          if (res && res.data && res.data.errors) {
            ...
            throw { message: res.data && res.data.message ? res.data.message : 'TRANSMISSION FAILED' };
          }
