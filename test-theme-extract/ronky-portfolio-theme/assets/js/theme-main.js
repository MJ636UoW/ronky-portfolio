/* ==========================================================================
  ...
  // ---------------------------------------------------------
  const contactForms = document.querySelectorAll('#portfolio-contact-form, .wp-portfolio-form');
  contactForms.forEach(contactForm => {
    if (contactForm.dataset.ajaxBound) return;
    ...
    contactForm.addEventListener('submit', (e) => {
      ...
      // Gather form data in Fluent Forms format
      const formIdRaw = contactForm.dataset.fluentFormId || '6';
      
      // Self-healing form ID extractor: extract numeric ID (e.g. from [fluentform id="6"])
      let formId = 6;
      const idMatch = formIdRaw.match(/id=["']?(\d+)["']?/);
      if (idMatch) {
        formId = parseInt(idMatch[1]);
      } else {
        formId = parseInt(formIdRaw.replace(/[^0-9]/g, '')) || 6;
      }

      // Gather input values
      const nameInput = contactForm.querySelector('[name="name"]') || contactForm.querySelector('#form-name, #wp-form-name');
      const emailInput = contactForm.querySelector('[name="email"]') || contactForm.querySelector('#form-email, #wp-form-email');
      const phoneInput = contactForm.querySelector('[name="phone"]') || contactForm.querySelector('#form-phone, #wp-form-phone');
      const serviceInput = contactForm.querySelector('[name="service"]') || contactForm.querySelector('#form-service, #wp-form-service');
      const budgetInput = contactForm.querySelector('[name="budget"]') || contactForm.querySelector('#form-budget, #wp-form-budget');
      const messageInput = contactForm.querySelector('[name="message"]') || contactForm.querySelector('#form-message, #wp-form-message');
      ...
      // Build data query string (as Fluent Forms expects)
      const fieldsData = {
        name: name,
        email: email,
        phone: phone,
      ...
      // Convert fieldsData to query string (like name=Mandar+Joshi&email=...)
      const queryString = Object.keys(fieldsData)
        .map(key => encodeURIComponent(key) + '=' + encodeURIComponent(fieldsData[key]))
        .join('&');
      ...
      const pageIdInput = contactForm.querySelector('[name="page_id"]');
      const elementIdInput = contactForm.querySelector('[name="element_id"]');
      if (pageIdInput) {
        formData.append('page_id', pageIdInput.value);
      }
      if (elementIdInput) {
        formData.append('element_id', elementIdInput.value);
      }

      // Fetch URL
      const ajaxUrl = typeof genz_ajax !== 'undefined' ? genz_ajax.ajax_url : '/wp-admin/admin-ajax.php';
      ...
      .then(res => {
        // Fluent Forms native handler returns JSON containing 'errors' or 'message' on failure.
        // On success, it typically returns a redirect URL or message and HTTP 200.
        // We check if there are validation errors.
        if (res && res.data && res.data.errors) {
          throw { validationErrors: res.data.errors };
        } else if (res && res.errors) {
          throw { validationErrors: res.errors };
        }
        ...
        if (err && err.validationErrors) {
          ...
          if (firstKey) {
            ...
            if (Array.isArray(fieldErrors) && fieldErrors.length > 0) {
              ...
              errorMsg = `${firstKey} IS REQUIRED`;
            }
          }
        }
        
        submitBtn.textContent = errorMsg.toUpperCase();
        submitBtn.style.color = '#FF3333';
